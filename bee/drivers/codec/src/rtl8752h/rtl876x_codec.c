...
/**
  * \brief  Store CODEC register values when system enter DLPS.
  * \param  PeriReg: Specifies to select the CODEC peripheral.
  * \param  StoreBuf: Store buffer to store CODEC register data.
  * \return None.
  */
DATA_RAM_FUNCTION
void CODEC_DLPSEnter(void *PeriReg, void *StoreBuf)
{
    CODECStoreReg_TypeDef *store_buf = (CODECStoreReg_TypeDef *)StoreBuf;

    RCC_PeriphClockCmd(APBPeriph_CODEC, APBPeriph_CODEC_CLOCK, ENABLE);

    store_buf->codec_reg[0] = CODEC->CR0;
    store_buf->codec_reg[1] = CODEC->ADC0_CTRL0;
    store_buf->codec_reg[2] = CODEC->ADC0_CTRL1;
    store_buf->codec_reg[3] = CODEC->ANA_CR1;
    store_buf->codec_reg[4] = CODEC->CLK_CR1;
    store_buf->codec_reg[5] = CODEC->CLK_CR2;
    store_buf->codec_reg[6] = CODEC->CLK_CR3;
    store_buf->codec_reg[7] = CODEC->I2S_CTRL;
}

/**
  * \brief  Restore CODEC register values when system exit DLPS.
  * \param  PeriReg: Specifies to select the CODEC peripheral.
  * \param  StoreBuf: Restore buffer to restore CODEC register data.
  * \return None
  */
DATA_RAM_FUNCTION
void CODEC_DLPSExit(void *PeriReg, void *StoreBuf)
{
    CODECStoreReg_TypeDef *store_buf = (CODECStoreReg_TypeDef *)StoreBuf;
    ...
    /* Initialize CODEC */
    if ((store_buf->codec_reg[3] & 0x900) == 0x900)
    {
        /* Configure AMIC parameters */
        CODEC->CR0 = store_buf->codec_reg[0];
    }

    CODEC->ADC0_CTRL0 = store_buf->codec_reg[1];
    CODEC->ADC0_CTRL1 = store_buf->codec_reg[2];
    CODEC->ANA_CR1    = store_buf->codec_reg[3];
    CODEC->CLK_CR1    = store_buf->codec_reg[4];
    CODEC->CLK_CR2    = store_buf->codec_reg[5];
    CODEC->CLK_CR3    = store_buf->codec_reg[6];
    CODEC->I2S_CTRL   = store_buf->codec_reg[7];
}
