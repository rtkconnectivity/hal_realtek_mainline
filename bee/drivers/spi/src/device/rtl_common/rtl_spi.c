...
#endif
#if (SPI1_SUPPORT_HS == 1)
extern void SPI_SPI1AHBBridgeConfig(FunctionalState NewState);
#endif
#if (SPI2_SUPPORT_HS == 1)
extern void SPI_SPI2AHBBridgeConfig(FunctionalState NewState);
#endif
/*============================================================================*
 *                           Public Functions
 *============================================================================*/
...
void SPI_DeInit(SPI_TypeDef *SPIx)
{
    ...
    if (SPIx == SPI0)
    {
        RCC_ClockCmd(SPI0_CLOCK, DISABLE);
        return;
    }
#endif
#ifdef SPI0_SLAVE
    if (SPIx == SPI0_SLAVE)
    {
        RCC_ClockCmd(SPI0_SLAVE_CLOCK, DISABLE);
        return;
    }
#endif
#ifdef SPI1
    if (SPIx == SPI1)
    {
        RCC_ClockCmd(SPI1_CLOCK, DISABLE);
        return;
    }
#endif
#ifdef SPI2
    if (SPIx == SPI2)
    {
        RCC_ClockCmd(SPI2_CLOCK, DISABLE);
        return;
    }
#endif
#ifdef SPI3
    if (SPIx == SPI3)
    {
        RCC_ClockCmd(SPI3_CLOCK, DISABLE);
        return;
    }
#endif
}
...
void SPI_Init(SPI_TypeDef *SPIx, SPI_InitTypeDef *SPI_InitStruct)
{
    ...
    if (IS_SPIM_PERIPH(SPIx))
    {
        ...
        if (SPIx == SPI1)
        {
            SPI_SPI1AHBBridgeConfig(DISABLE);
        }
        else if (SPIx == SPI1_HS)
        {
            SPI_SPI1AHBBridgeConfig(ENABLE);
        }
#endif
#if (SPI2_SUPPORT_HS == 1)
        if (SPIx == SPI2)
        {
            SPI_SPI2AHBBridgeConfig(DISABLE);
        }
        else if (SPIx == SPI2_HS)
        {
            SPI_SPI2AHBBridgeConfig(ENABLE);
        }
#endif
                 ...
                 (SPI_InitStruct->SPI_Direction == SPI_DIRECTION_FULLDUPLEX))
        {
            if (SPI_InitStruct->SPI_WrapModeEn)
            {
                ...
                spi_0x204.b.tx_fifo_tl = SPI_InitStruct->SPI_WrapTxThresholdLevel;
                ...
                spi_0x224.b.dma_tx_dl = SPI_InitStruct->SPI_WrapTxWaterlevel;
        ...
        SPI_WRAP_CS_DLY_TypeDef spi_0x22c = {.d32 = SPIx->SPI_WRAP_CS_DLY};
        spi_0x22c.b.cs_clk_div_sel = SPI_InitStruct->SPI_WrapCSClockDiv;
        spi_0x22c.b.cs_hold_dly = SPI_InitStruct->SPI_WrapCSHoldDlyCount;
        spi_0x22c.b.cs_setup_dly = SPI_InitStruct->SPI_WrapCSSetupDlyCount;
        SPIx->SPI_WRAP_CS_DLY = spi_0x22c.d32;
#endif
    }

#if (SPI0_SUPPORT_MASTER_SLAVE == 0) && defined(SPI0_SLAVE)
    /* Configure SPI Slave mode if select SPI0_SLAVE */
    else if (SPIx == SPI0_SLAVE)
    {
#if (SPI_SLAVE_SUPPORT_APH_BRIDGE_FOR_HIGH_SPEED == 1)
        SPI_ConfigAPHBridge(SPIx);
#endif
...
void SPI_StructInit(SPI_InitTypeDef *SPI_InitStruct)
{
    ...
    /* Wrapper Tx fifo depth: SPI_WRAPPER_TX_FIFO_SIZE bytes */
    SPI_InitStruct->SPI_WrapTxThresholdLevel  = 1;
    /* SPI Tx waterlevel, should be less than fifo threshold.
       The best value is SPI FIFO Depth - GDMA Msize */
    SPI_InitStruct->SPI_WrapTxWaterlevel   =  SPI_TX_FIFO_SIZE - 1;
    /* Set CS clock divider */
    SPI_InitStruct->SPI_WrapCSClockDiv = SPI_WRAP_CS_CLOCK_DIV_1;
    /* Set CS hold delay count */
    SPI_InitStruct->SPI_WrapCSHoldDlyCount = 0;
    /* Set CS setup delay count */
    SPI_InitStruct->SPI_WrapCSSetupDlyCount = 0;

#endif
    /* Tx fifo depth: SPI_TX_FIFO_SIZE bytes */
    SPI_InitStruct->SPI_TxThresholdLevel  = 1;
    /* Rx fifo depth: SPI_RX_FIFO_SIZE bytes */
    SPI_InitStruct->SPI_RxThresholdLevel  = 0;
    /* Disable dma */
    SPI_InitStruct->SPI_TxDMAEn           = DISABLE;
    SPI_InitStruct->SPI_RxDMAEn           = DISABLE;
    /* SPI Tx waterlevel, should be less than fifo threshold.
       The best value is SPI FIFO Depth - DMA Msize */
                ...
                FunctionalState NewState)
{
    ...
    if ((SPI_DMAReq == SPI_DMA_REQ_TX) || (SPI_DMAReq == SPI_DMA_REQ_RX))
    {
        if (NewState != DISABLE)
        {
            /* Enable the selected SPI DMA request */
            SPIx->SPI_DMACR |= SPI_DMAReq;
        }
        else
        {
            /* Disable the selected SPI DMA request */
    ...
    else
    {
        ...
        if (spi_0x200.b.wrap_ctrl_mode && SPI_NOT_SLAVE(SPIx))
        {
            /* enable tx ndf dma */
            SPI_WRAP_DMACR_TypeDef spi_0x220 = {.d32 = SPIx->SPI_WRAP_DMACR};
            spi_0x220.b.tx_dma_en = NewState;
            SPIx->SPI_WRAP_DMACR = spi_0x220.d32;
        }
    }
#endif
...
void SPI_WrapModeSetTxNdf(SPI_TypeDef *SPIx, uint16_t tx_ndf)
{
    ...
    SPI_WRAP_CTRL_TypeDef spi_0x200 = {.d32 = SPIx->SPI_WRAP_CTRL};
    spi_0x200.b.mst_tx_ndf = tx_ndf - 1;
    SPIx->SPI_WRAP_CTRL = spi_0x200.d32;
}
                           ...
                           uint16_t dummy_len)
{
    ...
    SPI_WRAP_TTRAN_NUM_TypeDef spi_0x238 = {.d32 = SPIx->SPI_WRAP_TTRAN_NUM};
    spi_0x238.b.task_tran_num = dummy_len;
    SPIx->SPI_WRAP_TTRAN_NUM = spi_0x238.d32;
}
...
void SPI_SetRepeatMode(SPI_TypeDef *SPIx, SPIRepeatMode_TypeDef mode)
{
    ...
    SPI_WRAP_CTRL_TypeDef spi_0x200 = {.d32 = SPIx->SPI_WRAP_CTRL};
    spi_0x200.b.repeat_reg_mode = mode;
    SPIx->SPI_WRAP_CTRL = spi_0x200.d32;
}
...
void SPI_SendRepeatData(SPI_TypeDef *SPIx, uint32_t data_l, uint32_t data_h)
{
    ...
    SPIx->SPI_WRAP_DATA_0 = data_l;
    SPIx->SPI_WRAP_DATA_1 = data_h;
}
