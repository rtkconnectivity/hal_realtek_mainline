...
{
    uint8_t index = I2C_GetIndex(I2Cx);
    uint32_t time_out_us = I2C_TimeOutUs[index];
    uint32_t start_timestamp_us = 0;
    uint32_t now_timestamp_us = 0;
     ...
     */
    start_timestamp_us = timestamp_get_us();
    while ((I2Cx->IC_STATUS & I2C_FLAG) == 0)
    {
        ...
        now_timestamp_us = timestamp_get_us();
        if ((uint32_t)(now_timestamp_us - start_timestamp_us) >= time_out_us)
        {
