...
/**
  * \brief  Store I2S register values when system enter DLPS.
  * \param  PeriReg: Specifies to select the I2S peripheral.
  * \param  StoreBuf: Store buffer to store I2S register data.
  * \return None.
  */
DATA_RAM_FUNCTION
void I2S_DLPSEnter(void *PeriReg, void *StoreBuf)
{
    I2SStoreReg_TypeDef *store_buf = (I2SStoreReg_TypeDef *)StoreBuf;

    RCC_PeriphClockCmd(APBPeriph_I2S0, APBPeriph_I2S0_CLOCK, ENABLE);

    store_buf->i2s_reg[0] = I2S0->BCLK_DIV;
    store_buf->i2s_reg[1] = I2S0->CTRL0;
    store_buf->i2s_reg[2] = I2S0->CTRL1;
    store_buf->i2s_reg[3] = I2S0->DMA_TRDLR;
}

/**
  * \brief  Restore I2S register values when system exit DLPS.
  * \param  PeriReg: Specifies to select the I2S peripheral.
  * \param  StoreBuf: Restore buffer to restore I2S register data.
  * \return None
  */
DATA_RAM_FUNCTION
void I2S_DLPSExit(void *PeriReg, void *StoreBuf)
{
    I2SStoreReg_TypeDef *store_buf = (I2SStoreReg_TypeDef *)StoreBuf;
    ...
    /* Initialize I2S0 */
    I2S0->BCLK_DIV  = store_buf->i2s_reg[0];
    I2S0->CTRL1     = store_buf->i2s_reg[2];
    I2S0->DMA_TRDLR = store_buf->i2s_reg[3];
    I2S0->CTRL0     = store_buf->i2s_reg[1];
}
